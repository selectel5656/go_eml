...
При первом запуске приложение создаёт таблицы в MySQL. Если база была создана в старой версии, столбец `key` в таблице `settings` автоматически переименовывается в `name`.
...
## Запуск на Windows
1. Установите [Go](https://go.dev/dl/), MySQL и LibreOffice. Убедитесь, что каталог с `libreoffice` добавлен в переменную `PATH`, иначе конвертация DOCX→PDF завершится ошибкой.
...
- Аттачи (изображения можно встраивать внутрь письма; при необходимости к ним добавляются случайные отступы и цветные точки. Для DOCX поддерживается добавление случайных страниц, вставка текста с макросами и опциональная конвертация в PDF. Для конвертации используется системный `libreoffice` (должен быть доступен в `PATH`). При загрузке документа из архива удаляются все отношения типа `stylesWithEffects` из любых файлов `.rels`, предотвращая ошибку "unsupported relationship type". Для каждого файла выдаются макросы `{{$attach_N}}` и `{{$attach_img_N_base64}}` для HTML)
- API правила
